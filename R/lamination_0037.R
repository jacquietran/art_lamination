...
# Modifiable params
iteration_id <- "lamination_0037"
initial_seed <- 5787037
grid_width <- 70
grid_height <- 1
plot_bg <- "#131313"
plot_res <- 100
...
# Gradient colours
base_colours <- c("#50B99B", "#DC244B", "#F3B816", "#F05A29")

gradient_colours1 <- c(base_colours, "#73B7B8", "#AF1D3C")
set.seed(initial_seed)
gradient_source1 <- sample(
  gradient_colours1, size = length(gradient_colours1), replace = FALSE)

gradient_colours2 <- c(base_colours, "#F6CB52", "#52A1A3")
set.seed(initial_seed)
gradient_source2 <- sample(
  gradient_colours2, size = length(gradient_colours2), replace = FALSE)

gradient_colours3 <- c(base_colours, "#76C8B1", "#D23F0F")
set.seed(initial_seed)
