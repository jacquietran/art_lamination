...
# Modifiable params
iteration_id <- "lamination_0028"
initial_seed <- 5787028
grid_width <- 100
grid_height <- 1
plot_bg <- "#131313"
plot_res <- 100
...
# Gradient colours
base_colours <- c("#46494C", "#4C5C68", "#1985A1", "#6FA4B4", "#9AB4BD")

gradient_colours1 <- c(base_colours, "#DCCCA3", "#E6C229")
set.seed(initial_seed)
gradient_source1 <- sample(
  gradient_colours1, size = length(gradient_colours1), replace = FALSE)

gradient_colours2 <- c(base_colours, "#E88D67", "#BB999C")
set.seed(initial_seed)
gradient_source2 <- sample(
  gradient_colours2, size = length(gradient_colours2), replace = FALSE)

gradient_colours3 <- c(base_colours, "#D5B9B2", "#A26769")
set.seed(initial_seed)
