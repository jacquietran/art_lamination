...
# Modifiable params
iteration_id <- "lamination_0032"
initial_seed <- 5787032
grid_width <- 70
grid_height <- 1
plot_bg <- "#131313"
plot_res <- 100
...
# Gradient colours
base_colours <- c("#481D24", "#872331", "#C5283D", "#E9724C", "#FFC857")

gradient_colours1 <- c(base_colours, "#1C77C3", "#39A9DB")
set.seed(initial_seed)
gradient_source1 <- sample(
  gradient_colours1, size = length(gradient_colours1), replace = FALSE)

gradient_colours2 <- c(base_colours, "#6DA34D", "#548687")
set.seed(initial_seed)
gradient_source2 <- sample(
  gradient_colours2, size = length(gradient_colours2), replace = FALSE)

gradient_colours3 <- c(base_colours, "#20A39E", "#19535F")
set.seed(initial_seed)
