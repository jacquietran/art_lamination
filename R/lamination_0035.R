...
# Modifiable params
iteration_id <- "lamination_0035"
initial_seed <- 5787035
grid_width <- 70
grid_height <- 1
plot_bg <- "#131313"
plot_res <- 100
...
# Gradient colours
base_colours <- c("#ABEDC6", "#D65780", "#F19A3E", "#1A5E63", "#654D72")

gradient_colours1 <- c(base_colours, "#81A4CD", "#39304A")
set.seed(initial_seed)
gradient_source1 <- sample(
  gradient_colours1, size = length(gradient_colours1), replace = FALSE)

gradient_colours2 <- c(base_colours, "#86BBD8", "#758E4F")
set.seed(initial_seed)
gradient_source2 <- sample(
  gradient_colours2, size = length(gradient_colours2), replace = FALSE)

gradient_colours3 <- c(base_colours, "#580C1F", "#FFE2FE")
set.seed(initial_seed)
...
ggplot2::ggsave(
  ...
  plots_in_grid, width = 7000, height = 100, units = "px",
...
img_rescaled <- img |>
  ...
  magick::image_morphology(method = "Dilate", iterations = 2, kernel = "Disk") |>
  magick::image_morphology(method = "Smooth", iterations = 5, kernel = "Disk") |>
