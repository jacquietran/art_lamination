...
# Modifiable params
iteration_id <- "lamination_0014"
initial_seed <- 5787014
grid_width <- 100
grid_height <- 1
plot_bg <- "#131313"
plot_res <- 100
...
# Gradient colours
base_colours <- c("#8ED081", "#B4D2BA", "#DCE2AA", "#B57F50", "#4B543B")

gradient_colours1 <- c(base_colours, "#EE4266", "#2A1E5C")
set.seed(initial_seed)
gradient_source1 <- sample(
  gradient_colours1, size = length(gradient_colours1), replace = FALSE)

gradient_colours2 <- c(base_colours, "#E26D5A", "#EFA8B8")
set.seed(initial_seed)
gradient_source2 <- sample(
  gradient_colours2, size = length(gradient_colours2), replace = FALSE)

gradient_colours3 <- c(base_colours, "#4E598C", "#FF8C42")
set.seed(initial_seed)
