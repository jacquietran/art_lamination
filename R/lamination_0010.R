...
# Modifiable params
iteration_id <- "lamination_0010"
initial_seed <- 5787010
grid_width <- 100
grid_height <- 1
plot_bg <- "#131313"
plot_res <- 100
...
# Gradient colours
base_colours <- c("#00BFB2", "#028090", "#F0F3BD", "#CA5369", "#BC386B")

gradient_colours1 <- c(base_colours, "#71A9F7", "#336699")
set.seed(initial_seed)
gradient_source1 <- sample(
  gradient_colours1, size = length(gradient_colours1), replace = FALSE)

gradient_colours2 <- c(base_colours, "#FF8811", "#DE603E")
set.seed(initial_seed)
gradient_source2 <- sample(
  gradient_colours2, size = length(gradient_colours2), replace = FALSE)

gradient_colours3 <- c(base_colours, "#F7D6E0", "#F2B5D4")
set.seed(initial_seed)
