...
# Modifiable params
iteration_id <- "lamination_0019"
initial_seed <- 5787019
grid_width <- 100
grid_height <- 1
plot_bg <- "#131313"
plot_res <- 100
...
# Gradient colours
base_colours <- c("#FFFEFF", "#FF80FD", "#FF01FB", "#8155F3", "#02A9EA")

gradient_colours1 <- c(base_colours, "#7ED475", "#FAFF00")
set.seed(initial_seed)
gradient_source1 <- sample(
  gradient_colours1, size = length(gradient_colours1), replace = FALSE)

gradient_colours2 <- c(base_colours, "#B4EDD2", "#C33C54")
set.seed(initial_seed)
gradient_source2 <- sample(
  gradient_colours2, size = length(gradient_colours2), replace = FALSE)

gradient_colours3 <- c(base_colours, "#F79256", "#FBD1A2")
set.seed(initial_seed)
